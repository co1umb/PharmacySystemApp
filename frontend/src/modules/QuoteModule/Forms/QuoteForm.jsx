...
function LoadQuoteForm({ subTotal = 0, current = null }) {
  ...
  useEffect(() => {
    if (current) {
      const { discountRate = 0, year, number } = current;
      setdiscountRate(discountRate / 100);
      setCurrentYear(year);
      setLastNumber(number);
    }
  }, [current]);
  useEffect(() => {
    const currentTotal = calculate.add(calculate.multiply(subTotal, discountRate), subTotal);
    setdiscountTotal(Number.parseFloat(calculate.multiply(subTotal, discountRate)));
          ...
          <Col className="gutter-row" span={4} offset={15}>
            ...
            >
              <SelectAsync
                ...
                entity={'discountes'}
                outputValue={'discountValue'}
                displayLabels={['discountName']}
                withRedirect={true}
                urlToRedirect="/discountes"
                redirectLabel={translate('Add New discount')}
                placeholder={translate('Select discount Value')}
              />
            </Form.Item>
          </Col>
          <Col className="gutter-row" span={5}>
